...
We have manually written a failing test to check if the failed test fails the build or not. So, remove ```node testgen.js``` from Execute Shell Command for this build so that the failing test in ```test.js``` is not over written by the test cases generated by the testgen script.
...
Go to 'Publish Checkstyle analysis results' under 'Post Build Actions'. Click on 'Advanced' button and under the 'Status Thresholds' enter 2 for 'All Priorities' column so that it fails the minimum analysis criteria since we have 3 warnings given by checkstyle. That is, the plugin will check the number of warnings raised by the code and compare it to the Status Threshold and if the number of warnings is more than the threshold, the build will fail.
...
We created a new file and added the key to it to check if the capability is able to parse the files staged for commit and reject the commit if key is found.
